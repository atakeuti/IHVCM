...
{
    public class cINaCajs : cElement
    {
        ...
        override public void Initialize(ref double[] myTVc, ref cCell myCell, ref ListForm Lf)
        {
            ...
            Lf.DGViews_Initialize(Lf.tpINaCajs_ListView, NumOfIx);

            Itc = myTVc[Pd.InxINaCajs_Itc];
        }
        ...
        override public void dydt(double dt, ref double[] tvDYdt, ref double[] tvY, cCell myCell)
        {
            ...
            Itc = SF * myCell.JS.fraction_js * ANCX * Ka * (s1 - s2) / (s3 * (1 + ksat * Exp_nu_1_VFRT));
            myCell.TVc[Pd.InxINaCajs_Itc] = Itc;
        }

        override public void DispValues(ref ListForm Lf, ref double[] myTVc)
        {
            ucListView ListView = Lf.tpINaCajs_ListView;
